...
class UsersAssignedRoleDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('user', function (User $user) {
                return view('admin/apps.user-management.roles.columns._user', compact('user'));
            })
            ->editColumn('created_at', function (User $user) {
                return $user->created_at->format('d M Y, h:i a');
            })
            ->addColumn('action', function (User $user) {
                return view('admin/apps.user-management.roles.columns._actions', compact('user'));
            })
            ->setRowId('id');
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
            ...
            ->orderBy(1)
            ->drawCallback("function() {" . file_get_contents(resource_path('views/admin/apps/user-management/users/columns/_draw-scripts.js')) . "}");
    }
